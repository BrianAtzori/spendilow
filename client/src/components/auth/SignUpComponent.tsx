  ...
  const handleProfileImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onerror = (error) => {
        alert(`Errore nel caricare l'immagine! Contatta il supporto. Errore: ${error}`);
      };

      reader.readAsDataURL(file);
    } else {
      alert(
        'Immagina non caricata correttamente! Riprova, cambia immagine oppure contatta il supporto.',
      );
    }
  };
    ...
    <div className='hero min-h-screen text-neutral'>
      <div className='hero-content flex-col desktop:flex-row-reverse'>
        ...
        <div className='card shrink-0 w-full max-w-sm shadow-2xl bg-base-100'>
          <form className='card-body font-body' onSubmit={verifyInputThenTriggerSignUp}>
