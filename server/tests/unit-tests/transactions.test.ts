...
let spendilowTestingTransactionsUser: any;
let spendilowTestTransaction: any;
...
describe("Spendilow API 💰 - Transactions Tests", function () {
  ...
  it("should get a single user transaction given the ID on /authenticated-users/transactions/get/:id GET", function (done) {
    chaiTests
      .request(baseURL)
      .get(
        `api/v1/authenticated-users/transactions/get/${singleSpendilowTestTransaction}`
      )
      .set("Cookie", cookie)
      .end(function (err: Error, res: any) {
        res.should.have.status(200);
  ...
  it("should edit a single user transaction given the ID on /authenticated-users/transactions/mod/:id", function (done) {
    chaiTests
      .request(baseURL)
      .patch(
        `api/v1/authenticated-users/transactions/mod/${singleSpendilowTestTransaction}`
      )
      .send({
        amount: 10,
        transaction_date: "1970/01/01",
  ...
  it("should delete a single user transaction given the ID on /authenticated-users/transactions/del/:id", function (done) {
    chaiTests
      .request(baseURL)
      .delete(
        `api/v1/authenticated-users/transactions/del/${singleSpendilowTestTransaction}`
      )
      .set("Cookie", cookie)
      .end(function (err: Error, res: any) {
        res.should.have.status(200);
